...
export const createUser = (req, res) => {
  const { user_name, user_age, user_dissease, user_email, user_password } =
    req.body;
  const query =
    "INSERT INTO users (user_name, user_age, user_disease, user_email, user_password) VALUES (?, ?, ?, ?, ?)";
  ...
  db.query(emailCheckQuery, [user_email], (err, result) => {
    ...
    db.query(
      query,
      [user_name, user_age, user_dissease, user_email, user_password],
      (err, result) => {
        if (err) {
          return res.status(500).json({ error: err.message });
        }
