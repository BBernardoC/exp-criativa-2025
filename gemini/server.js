...
if (!API_KEY) {
  console.error(
    "Erro: A chave de API GOOGLE_API_KEY não foi definida no arquivo .env."
  );
  process.exit(1);
}
...
app.post("/api/gemini-chat", async (req, res) => {
  ...
  const geminiPrompt = `
    ...
    Escreva o conteúdo de forma que seja acessivel a pessoas com menos informação e conhecimento técnico,
    mas mantenha a precisão e clareza necessárias para que o usuário possa entender e utilizar a informação fornecida. 
  ...
  try {
    ...
    if (parsedResponse.title && parsedResponse.text) {
      ...
      console.warn(
        'JSON do Gemini não contém "title" ou "text" conforme esperado.',
        parsedResponse
      );
      res
        .status(500)
        .json({ error: "Resposta incompleta da IA. Tente novamente." });
    }
  } catch (error) {
